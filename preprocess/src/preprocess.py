...
from common.utils import get_tag_from_input_parameters, list_input_parameters
...
def main(cfg: DictConfig):
    input_parameters = list_input_parameters(cfg.target_parameters)
    mlflow.set_tag("mlflow.runName", get_tag_from_input_parameters(input_parameters) + "_preprcess")
    ...
    if cfg.debug_mode is True:
        logger.info("This is debug mode and do nothing in preprocess flow.")
        return
    ...
    mlflow.log_artifacts(
        downstream_dir_path,
        artifact_path="downstream_dir_path",
    )
    logger.info(f"meta info files have saved in {downstream_dir_path}")
